  ...
  Widget build(BuildContext context) {
    ...
    Future<void> downloadTheSurah() async {
      try {
        ...
        for (int i = 0; i < versesPositions.length; i++) {
          File audioFile = File('${audioDir.path}/${i + 1}.mp3');
          if (!audioFile.existsSync()) {
            final task = DownloadTask(
              url:
                  'https://cdn.islamic.network/quran/audio/64/${alQuranInfoProvider.reciter.split('~')[1]}/${versesPositions[i]}.mp3',
              filename: '${i + 1}.mp3',
              directory: audioDir.path,
              baseDirectory: BaseDirectory.root,
              allowPause: true,
              group: 'surah-$surahId-${alQuranInfoProvider.reciter}',
            );
        ...
        FileDownloader().configureNotificationForGroup(
          'surah-$surahId-${alQuranInfoProvider.reciter}',
          running: const TaskNotification(
              '{numFinished} / {numTotal}', '{progress}'),
          complete: const TaskNotification('Download Complete!', ''),
          error: const TaskNotification('Downlod Failed!', ''),
          progressBar: true,
          groupNotificationId:
              'downloading-surah-$surahId-${alQuranInfoProvider.reciter}',
        );
